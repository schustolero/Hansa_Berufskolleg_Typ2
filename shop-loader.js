...
(function(){
  ...
  function loadFileFallback(callback){
    ...
    script.src = `/shops/${encodeURIComponent(slug)}/shop-config.js?v=28.5.2`;
  ...
  window.loadShopConfig = async function(callback){
    ...
    const seedFallback = (central.seedShops && central.seedShops[slug]) || null;
    if (seedFallback) {
      window.SHOP_CONFIG = seedFallback;
      callback && callback(seedFallback);
      return;
    }
